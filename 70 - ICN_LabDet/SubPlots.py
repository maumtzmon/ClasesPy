...
#ICN
sys.path.insert(0, '/home/oem/Software/Serial_Register_Events_Detection') #declarar este directorio,importa una libreria cuya dirección es conocida 
...
#cambia la dirección por la que quieras al rato
#solo tenemos una direccion hasta ahora
hdu_list = fits.open(path) #fits abre imagenes, es de astropy
print(hdu_list.info())
print('----------------')
hdu_list[0].header
#header me da la información más relevante del archivo

fig,axs=plt.subplots(ncols=2,nrows=2, sharex=True, sharey=True,figsize=(20,10)) #plt.subplot regresará figura con axs ndimensional dependiendo de lo que le ingreses a plt.subplot
#default 1 axs, define cols y rows
#figsize en inch

raw_1=axs[0][0].imshow(hdu_list[0].data, vmin=np.median(hdu_list[0].data)-800,vmax=np.median(hdu_list[0].data)+800) #imshow despliega matriz como imagen
#has ejercicio de quitarvmin y vmax
...
# top_right=axs[0][1].hist(raw_data[3][100:300,550:].flatten(), bins=3000)
# bottom_right=axs[1][1].hist(data_precal[3][100:300,550:].flatten(), bins=3000)
